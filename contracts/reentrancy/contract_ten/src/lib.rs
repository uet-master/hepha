...
pub fn process_instruction(
    ...
    match instruction {
        ...
        1 => {
            let range = u64::from_le_bytes(instruction_data[1..9].try_into().unwrap());
            withdraw_random_amount(&mut balances, user_account.key, range, user_account, contract_account)?;
        }
        _ => {
            msg!("Invalid action");
